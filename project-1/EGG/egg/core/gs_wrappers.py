...
class RnnSenderGS(nn.Module):
    ...
    def forward(self, x):
        ...
        #force model to use full message length
        swap = sequence[:,:-1,1][sequence[:,:-1,0]>.5].clone()
        sequence[:,:-1,1][sequence[:,:-1,0]>.5] = sequence[:,:-1,0][sequence[:,:-1,0]>.5].clone()
        sequence[:,:-1,0][sequence[:,:-1,0]>.5] = swap
        return sequence
...
class SenderReceiverRnnGS(nn.Module):
    ...
    True
    """
    def __init__(self, sender, receiver, loss, device, length_cost=0.0, baseline=False):
        """
    ...
    def forward(self, sender_input, labels, reference, distractors, receiver_input=None):
        if self.baseline:
          no_ref = torch.zeros(sender_input.size(0),1,28,28).to(self.device)
          message = self.sender((sender_input, no_ref))
          receiver_output = self.receiver(message, no_ref)
        else:
          message = self.sender((sender_input, reference))
          receiver_output = self.receiver(message, reference)
        ...
        rest = {}
        z = 0.0
        target_repr = self.receiver.agent.vision(sender_input)
        distract_reprs = move_to(torch.zeros((distractors.size(0),distractors.size(1),target_repr.size(1))),self.device)
        for i in range(distractors.size(1)):
          distract_reprs[:,i,:] = self.receiver.agent.vision(distractors[:,i,:,:,:])

        for step in range(receiver_output.size(1)):
            step_loss, step_rest = self.loss(sender_input, message[:, step, ...], receiver_input, receiver_output[:, step, ...], (target_repr, distract_reprs))
            eos_mask = message[:, step, 0]  # always eos == 0

            add_mask = eos_mask * not_eosed_before
            z += add_mask
            loss += step_loss * add_mask + (step + 1.0) * self.length_cost * add_mask
        ...
        loss += step_loss * not_eosed_before + (step + 1.0) * self.length_cost * not_eosed_before
